...
export class GradeService {
  ...
  getGradesByClass(classId: number): Observable<Grade[]> {
    const headers = this.authService.getAuthHeaders();
    return this.http
      .get<Grade[]>(`${this.API_URL}/Grade/class/${classId}`, { headers })
  ...
  getGradesByAssignment(assignmentId: number): Observable<Grade[]> {
    const headers = this.authService.getAuthHeaders();
    return this.http
      .get<Grade[]>(`${this.API_URL}/Grade/assignment/${assignmentId}`, {
        headers,
      })
      .pipe(catchError(this.handleError));
  }
  ...
  getGradeById(gradeId: number): Observable<Grade> {
    const headers = this.authService.getAuthHeaders();
    return this.http
      .get<Grade>(`${this.API_URL}/Grade/${gradeId}`, { headers })
      .pipe(catchError(this.handleError));
  }
  ...
  createGrade(gradeRequest: CreateGradeRequest): Observable<Grade> {
    ...
    return this.http
      .post<Grade>(`${this.API_URL}/Grade`, minimalPayload, { headers })
      .pipe(
        catchError((error) => {
          ...
          if (error.error) {
            ...
            if (
              details.includes('DateTime with Kind=Unspecified') ||
              details.includes('timestamp with time zone')
            ) {
              console.error(
                'PostgreSQL DateTime error detected - using alternative approach'
              );
          ...
          if (error.error && error.error.message) {
            ...
            errorMessage =
              'Invalid grade data. Please check all fields and try again.';
          }
  ...
  private createGradeAlternative(payload: any): Observable<Grade> {
    ...
    return this.http
      .post<Grade>(`${this.API_URL}/Grade`, stringPayload, { headers })
      .pipe(
        catchError((error) => {
          ...
          return throwError(
            () =>
              new Error(
                'Failed to create grade using alternative method. Please try again later.'
              )
          );
        })
      );
  ...
  createGradesBatch(gradeRequests: CreateGradeRequest[]): Observable<any> {
    ...
    return this.http
      .post<any>(`${this.API_URL}/Grade/batch`, preparedPayload, { headers })
      .pipe(
        catchError((error) => {
          ...
          if (error.error && error.error.message) {
            ...
            errorMessage =
              'Invalid grade data in batch. Please check all fields and try again.';
          }
  ...
  updateGrade(
    gradeId: number,
    gradeRequest: UpdateGradeRequest
  ): Observable<boolean> {
    const headers = this.authService.getAuthHeaders();
    ...
    return this.http
      .put<boolean>(`${this.API_URL}/Grade/${gradeId}`, payload, {
        headers,
  ...
  deleteGrade(gradeId: number): Observable<any> {
    ...
    return this.http
      .delete<any>(`${this.API_URL}/Grade/${gradeId}`, { headers })
      .pipe(
        catchError((error) => {
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          return throwError(
            () => new Error('Could not verify existing grades')
          );
        })
      );
  }
